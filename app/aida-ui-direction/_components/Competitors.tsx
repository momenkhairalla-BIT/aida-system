...
const COMPETITORS: Comp[] = [
  {
    ...
    learn:
      "Rewards and points are the hero of the home screen, with frictionless app-based ordering and constant reward visibility.",
...
export function Competitors() {
  return (
    <section className="section section--tint" id="benchmark">
      <div className="container">
        <div className="section-head">
          <span className="eyebrow">
            <span className="dot" />
            02 · Competitor-Inspired UX
          </span>
          <h2 className="section-title">Benchmarked against the best café apps</h2>
        ...
        <div className="comp-grid">
          {COMPETITORS.map((c) => (
            <article className="comp-card" key={c.name}>
              <span className="comp-card__accent" style={{ background: c.accent }} />
              <div className="comp-card__init" style={{ background: c.initBg }}>
                {c.init}
              </div>
              <div className="comp-card__name">{c.name}</div>
              <div className="comp-card__tag">{c.tag}</div>
              <p className="comp-card__learn">{c.learn}</p>
              <div className="comp-card__take">
                <IconArrow size={16} />
                <span>{c.take}</span>
              </div>
            </article>
